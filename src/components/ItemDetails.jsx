...
const ItemDetails = ({
                         formik, selectedItem, setSelectedItem, type, setRefetch,
                     }) => {
    ...
    return (<div className="flex flex-col gap-2 text-emerald-600 w-full bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        >
            <EditableField
                beforeEditData={selectedItem["title"] || selectedItem[type]}
                formik={formik}
                id={selectedItem["title"] ? "title" : type}
            ...
            {type === "report" && (<>
                <SelectBox
                    ...
                    id={"type"}
                    label={"Type"}
                    options={["error", "feedback", "suggestion", "other"]}
                />
            </>)}
            <EditableField
