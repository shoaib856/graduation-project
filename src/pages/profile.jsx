...
function Profile() {
    ...
    const profileLocation = location.pathname.split("/").slice(-2)[0] === "profile";

    document.title = profileLocation ? "FarmVision | Profile" : "Dashboard | User Details";
        ...
        try {
            if (profileLocation) {
                if (userData && !refetch) {
                    ...
                    await formik.setFieldValue("image", fromBase64ToImg(userData.image));
                    setUser(userData);
                } else if (refetch) {
                ...
                await axios
                    ...
                    .then((res) => {
                        ...
                        formik.setFieldValue("image",fromBase64ToImg(res.data.data.user.image));
                        setLoading(false);
                        setRefetch(false);
                        auth.id === params.id && refetch ? getData(auth, setError, setUserData, setRefetch) : null;
                    })
                    .catch((err) => {
                        console.error(err);
                        setError(true);
    ...
    return (<div className="w-full py-3 px-1 flex flex-col gap-4 items-center bg-white !rounded-xl shadow-xl">
        ...
        {error ? (<div className="w-full">
            ...
            />
        </div>) : !user || refetch ? (<Card className="w-full">
